...
if ($entity && ($owner_guid == elgg_get_logged_in_user_guid() || elgg_is_admin_logged_in())) {
	...
	$causes = get_groups_causes($entity->guid);
	if($causes){
		echo "<div>";
		echo "<label>Select featured cause</label></br>";
		echo elgg_view('input/dropdown', array(
			'name' => 'selected_cause',
			'value' =>  $selected_cause,
			'options_values' => $causes,
		));
		echo "</div>";
	}

	elgg_load_library('elgg:event');
	$events = get_groups_events($entity->guid);
	if($events){
		$featured_event = featured_event_of_group($entity->guid);
		if($featured_event){
			$selected_event = $featured_event->guid;
		}	
		echo "<div>";
		echo "<label>Select featured event</label></br>";
		echo elgg_view('input/dropdown', array(
			'name' => 'selected_event',
			'value' =>  $selected_event,
			'options_values' => $events,
		));
		echo "</div>";
	}	
