...
function event_compare($guid_one, $guid_two){
	...
	$sql= "SELECT distinct user_guid from $table WHERE event_guid=$guid_one and user_guid  in (select user_guid from $table where event_guid=$guid_two )";
	$data = get_data($sql);
	return $data;
...
$filename = "event_common_report.xls";
